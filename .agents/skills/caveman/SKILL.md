---
name: caveman
description: Use explicitly invoked /caveman or "caveman mode" for ultra-compressed communication that preserves technical accuracy; do not infer it from a generic request for brevity.
license: MIT
---

## BLUEVERSE compatibility

This skill changes conversational style only. It does not change project rules,
tool permissions, architecture, security, testing, validation, or authorization.

- Follow higher-priority host and repository instructions first. Required
  progress updates, approval warnings, validation evidence, and AI-usage
  records remain mandatory.
- Keep exact code, API names, paths, identifiers, error strings, status values,
  and numeric data.
- Keep normal prose in code comments, source files, documentation, commits,
  issues, pull requests, logs, and other saved artifacts.
- The `.claude/skills` copy targets Claude and Antigravity. The
  `.agents/skills` copy remains portable for agents that explicitly opt in.
...
Tool calls: fire direct. No optional preamble, plan, or progress note before or between calls. Higher-priority host or repository instructions may require concise progress updates. After result: next call direct or final answer never announce next call. Text before call only to clarify, warn security/irreversible, or resolve ambiguity.
...
- Required authorization, validation, audit, or contribution-record details
