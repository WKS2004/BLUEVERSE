...
## BLUEVERSE compatibility boundary

This compiled reference is vendored from Vercel Engineering for the
BLUEVERSE React 19 + Vite client. Read the project-owned
`.agents/skills/blueverse-client-contract/SKILL.md` and applicable rules first.
Ignore Next.js, React Server Components, server-action, route-handler and
server-only advice unless BLUEVERSE explicitly adopts those technologies.
Keep all client traffic behind the public `/api/...` ASP.NET Core boundary;
never use this document to add direct Auth, Agentic AI or PostgreSQL access.
