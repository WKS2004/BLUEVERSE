...
description: Adds Flutter integration tests with the current integration_test package and stable UI interactions. Use for end-to-end mobile workflows; use legacy flutter_driver guidance only when migrating an existing driver suite.
...
## BLUEVERSE compatibility

Apply this skill only to `apps/mobile` and its authoritative test paths. Read
`blueverse-client-contract` and `blueverse-test-design` first. Integration
tests must exercise the public `/api/...` gateway and preserve the shared
authorization contract; never call Auth, Agentic AI or PostgreSQL directly.
...
Configure the project to support Flutter's current `integration_test` package.
...
1. Add required development dependencies to `pubspec.yaml`:
   ...
   ```
2. Add `Key` parameters (e.g., `ValueKey('login_button')`) to critical
   widgets in the application code to ensure reliable targeting during tests.
3. Keep platform permissions, gateway URLs and test dependencies aligned with
   the current project; do not add a driver extension unless migrating a
   legacy `flutter_driver` suite.
...
Execute ordinary integration tests with `flutter test integration_test/<name>_test.dart`.
Use `flutter drive` and a `test_driver/` script only for an existing profiling
or legacy-driver workflow that the project explicitly needs.
...
- [ ] **Task Progress: Setup**
  ...
  - [ ] Use `IntegrationTestWidgetsFlutterBinding.ensureInitialized()` in the test entry point.
...
- [ ] **Task Progress: Authoring**
  ...
  - [ ] Add a `test_driver/` script only when a legacy/profiling driver workflow requires it.
- [ ] **Task Progress: Execution & Feedback Loop**
  - [ ] Run `flutter test integration_test/app_test.dart` on the supported target.
