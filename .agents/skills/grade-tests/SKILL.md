---
name: grade-tests
description: Grade supplied test methods individually with concise A-F feedback. Use for curated test review, not suite-wide audits, new tests, fixes or coverage measurement.
license: MIT
---
...
Read `blueverse-test-design` and the repository testing rules first. Grade only
the supplied tests; do not silently rewrite, delete or relax them. Use the
repository's stable IDs and complete-observable-behavior expectations when
explaining a score.
