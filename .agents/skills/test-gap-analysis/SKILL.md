...
description: Analyze caller-visible behavioral test gaps using pseudo-mutation reasoning. Use when a production change might pass existing tests, not for suite organization, assertion audits or new test writing.
license: MIT
---
...
Read `blueverse-test-design` and the repository testing rules first. Keep
requirement-based expectations independent from implementation details and
report missing service source as a foundation gap. Do not edit protected tests
