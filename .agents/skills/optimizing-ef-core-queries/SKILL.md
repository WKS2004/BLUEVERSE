...
description: Diagnose and optimize slow EF Core queries by reducing SQL, round trips, scans and translation cost. Use for EF Core data-access paths, not Dapper or raw ADO.NET.
license: MIT
---
...
Read `blueverse-backend-service`, architecture, security and database rules
first. Use this only for an implemented EF Core service path; PostgreSQL access
must remain behind the backend/service layer. Preserve required constraints,
indexes, audit fields, authorization filters and migration behavior. Do not
invent a `DbContext` or service while the current checkout has no tracked
backend project.
