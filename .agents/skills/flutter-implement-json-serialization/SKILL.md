...
Apply this to API-boundary models in `apps/mobile`. Read
`blueverse-client-contract` first and keep models aligned with the public API
contract shared by React and Flutter. Treat responses as untrusted input,
validate required fields and avoid placing business authorization rules in the
client.
