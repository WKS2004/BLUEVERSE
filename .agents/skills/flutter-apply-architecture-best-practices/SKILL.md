...
Apply this to `apps/mobile` only. Read `blueverse-client-contract` and the
architecture/security rules first. Data services may call only the public
ASP.NET API/gateway; do not introduce direct Auth, Agentic AI or PostgreSQL
access, and do not create a replacement sample application. Preserve the
repository's chosen state-management and dependency conventions when they
become established instead of blindly adding a package from the example.
