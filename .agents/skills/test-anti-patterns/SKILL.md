...
description: Audit test suites for missing or tautological assertions, swallowed exceptions, flakiness, duplication and magic values. Use for diagnostics, not direct test edits or coverage analysis.
license: MIT
---
...
Read `blueverse-test-design` and the repository testing rules first. Produce a
diagnostic report before proposing changes. Existing tests are protected
specifications; any change, deletion or relaxation requires user permission.
