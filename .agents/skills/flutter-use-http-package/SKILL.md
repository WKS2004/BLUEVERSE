...
Apply this to `apps/mobile` API clients only. Read `blueverse-client-contract`
and security rules first. Use the public `/api/...` gateway, treat tokens and
responses as sensitive/untrusted, preserve server-side authorization, and do
not hard-code secrets or internal service URLs. Match the repository's actual
dependency and test setup before running package-add commands.
