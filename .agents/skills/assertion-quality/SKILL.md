...
description: Analyze test assertion depth, variety and false confidence across supported frameworks. Use for assertion-quality reviews, not direct test writing or behavioral gap analysis.
license: MIT
---
...
Read `blueverse-test-design` and the repository testing rules first. Analyze
existing tests without weakening or rewriting their expectations. For missing
backend suites, report the foundation gap rather than generating a substitute
service or test project.
