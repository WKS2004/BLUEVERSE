---
name: run-tests
description: Choose and run repository-compatible .NET test commands, filters, diagnostics, TRX and coverage options. Use before answering or executing .NET test commands; not for writing tests or CI investigation.
license: MIT
metadata:
...
Read `blueverse-test-design` and `.agents/rules/validation.md` first. Use the
actual project/test paths discovered in the checkout and the pinned SDK from
`global.json`. If the backend project is absent, report the foundation gap
instead of inventing a command, project or test suite. Preserve complete test
outcomes and the repository rule protecting existing tests.
