---
name: dotnet-webapi
description: Create or modify ASP.NET Core Web API endpoints with correct HTTP semantics, OpenAPI metadata, validation, error handling and request tests. Use for API work, not EF Core, frontend, gRPC or SignalR.
license: MIT
---
...
Read `blueverse-backend-service` and the project rules first. Apply this skill
only when an actual ASP.NET project exists; do not scaffold a sample service
into a reserved path or treat `bin/`/`obj/` output as source. Preserve the
public `/api/...` convention, role-to-permission authorization, DHI runtime
assumptions and the repository's existing OpenAPI choice. Generic controller
or minimal-API recommendations are subordinate to the checked-in project.
