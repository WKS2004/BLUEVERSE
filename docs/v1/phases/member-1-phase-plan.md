# Ushan Srinuka (Member 1) phase plan — Coastal Experience & Biodiversity Discovery

**Assigned owner:** Ushan Srinuka (`@Ushan-Srinuka`). Feature branch:
`features/experience-biodiversity`; Agentic AI branch after G07:
`agentic-ai/experience-biodiversity`.

This plan divides the complete [Ushan Srinuka (Member 1) component
...
flows, implement Ushan Srinuka's business logic in its own `services/` microservice,
and limit `services/api` to integration code. Minimize shared React, Flutter,
route-registry and infrastructure edits.

Ushan Srinuka (Member 1) owns destinations, activities, offerings, schedules, publication and
availability, discovery, favourites, the selected map-provider adapter, and
the user-facing biodiversity context surface. Adithya Gunawardana (Member 3) owns BLUEVERSE's
backend adapter to the separate IT3091 inference API and its validated public
prediction contract; IT3091 supplies the model/inference service. Ushan Srinuka (Member 1)
consumes the Adithya Gunawardana (Member 3) public contract and does not call the private service.
Ushan Srinuka (Member 1) does not own environmental conditions/suitability, itinerary
...
`dev` after the component PRs merge. Ushan Srinuka (Member 1) can build its biodiversity
presentation against the agreed Adithya Gunawardana (Member 3) contract and controlled fixtures
while Adithya Gunawardana (Member 3) implements the inference adapter; verify the live consumer and
...
booking/payment inventory, direct client access to ML, or guessed Sanuda Abeysinghe (Member 2)
...
and activity taxonomy. Sanuda Abeysinghe (Member 2) uses the taxonomy for profiles; Wanshaja Sooriyabandara (Member 4)
references these IDs for managed operational targets; Adithya Gunawardana (Member 3) references
...
the Wanshaja Sooriyabandara (Member 4) status contract agreed at G00. Implement the consumer against
that contract and controlled test doubles while Wanshaja Sooriyabandara (Member 4) develops its branch;
...
- server-side consumption of Wanshaja Sooriyabandara's current authoritative status. If that
  status is unavailable or stale, preserve the defined uncertainty/failure
  result; do not maintain a competing Ushan Srinuka (Member 1) status field.
...
availability semantics for Adithya Gunawardana and Wanshaja Sooriyabandara. An initial Wanshaja Sooriyabandara (Member 4) status may be
...
- the selected map API integration through a server-side adapter owned by
  Ushan Srinuka's internal .NET service and reached by clients through the public
...
- the Ushan Srinuka (Member 1) public API workflow initiation/status contract, authorization,
...
- no production agent, prompt/model call, tool execution or fabricated report
  ...
  adapter belongs to Adithya Gunawardana (Member 3), is separate from Agentic AI, and must be ready
  before G07. Ushan Srinuka (Member 1) consumes its validated public result contract.
...
- Wanshaja Sooriyabandara (Member 4) restriction consumption and the Adithya Gunawardana (Member 3) candidate contract,
...
- React/Flutter role-action parity, the GPS-denied fallback, map-provider
  ...
  of Adithya Gunawardana's available/unavailable biodiversity result;
- route catalog, UI registry, OpenAPI, component docs, tests and attributable
  PR evidence.

**Ushan Srinuka (Member 1) exit:** the component meets its contract without an executable
Ushan Srinuka (Member 1) agent. The agent remains deferred until the team accepts G07.

## Progress record

Update the Ushan Srinuka (Member 1) row in the
