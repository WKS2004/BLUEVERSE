...
# Adithya Gunawardana (Member 3) — Smart Coastal Planner & Itinerary Management

**Contract status:** v1 target specification. The current foundation does not
contain this complete v1 planner. **Assigned owner:** Adithya Gunawardana
(`@AdithyaGunawardana`) — frozen-requirements trace label Member 3.
...
| **Adithya Gunawardana (Member 3) .NET service and data** | A separate internal ASP.NET Core service in Adithya Gunawardana's own `services/<component-service>/` subfolder owns planning requests, deterministic candidate assembly, workflow identity/status/result, itinerary persistence and the private IT3091 biodiversity inference adapter plus validated prediction-result operation consumed by Ushan Srinuka (Member 1). Its EF Core/PostgreSQL records are owned by this service; prediction caching/persistence is decided from freshness, privacy and retention needs. The existing `services/api` receives only authentication/permission, routing/forwarding and typed-integration code needed to expose public `/api/...` operations; it contains no Adithya Gunawardana (Member 3) business logic or ML calls. Ushan Srinuka, Sanuda Abeysinghe and Wanshaja Sooriyabandara source results remain authoritative. Clients never call IT3091 or internal services directly. Agree identifiers, routes, DTOs, actor/permission propagation and schema at G00. |
| **ML service integration** | Adithya Gunawardana (Member 3) sends minimal validated location/species/context to the private IT3091 service, validates response schema, numeric ranges, timestamps and provenance, and returns genuine predictions or a clear unavailable/invalid result. IT3091 supplies the trained model and inference service; Adithya Gunawardana (Member 3) does not train, host, or claim ownership of it. Ushan Srinuka's experience screens consume the Adithya Gunawardana (Member 3) public contract. Biodiversity is optional contextual enrichment, never a safety or operational authority, and this adapter is ordinary backend ML/API integration rather than Agentic AI. |
| **Other provider integration** | Adithya Gunawardana (Member 3) does not own the map provider or Open-Meteo. It consumes Ushan Srinuka's canonical catalogue/location/availability and Sanuda Abeysinghe's backend-mediated Open-Meteo conditions/suitability, plus Wanshaja Sooriyabandara (Member 4) restrictions. Only the typed Adithya Gunawardana (Member 3) server adapter may call IT3091. Neither client, planner UI, future model nor agent tool may call the map provider, Open-Meteo, IT3091, Auth or internal hosts directly. Map-assisted discovery remains in Ushan Srinuka (Member 1); planning receives validated Ushan Srinuka (Member 1) data. |
| **Paired Agentic AI role** | The future Planning & Coordination Agent creates a structured plan, delegates to the distinct Ushan Srinuka (Member 1) and Sanuda Abeysinghe (Member 2) specialists, tracks dependencies and assembles their validated outputs; it participates in Wanshaja Sooriyabandara (Member 4) assessment when that workflow requests it. Before G07, Adithya Gunawardana (Member 3) implements its ordinary deterministic business behavior, public workflow contract, typed private adapter and safe not-connected/unavailable state only. Actual orchestration/model calls and AI execution state wait for `agentic-ai/**` after G07. |
| **Component relationships** | Adithya Gunawardana (Member 3) consumes Ushan Srinuka (Member 1) publication/schedule/availability, Sanuda Abeysinghe (Member 2) condition freshness and suitability, and Wanshaja Sooriyabandara (Member 4) current restrictions. It owns the adapter that obtains optional biodiversity predictions from IT3091 and exposes validated context to Ushan Srinuka (Member 1); Ushan Srinuka (Member 1) owns experience-facing presentation. Its business workflow ID/objective/status/result references are consumed by Wanshaja Sooriyabandara (Member 4) for traceable assessment. It never replaces source ownership or weakens `UNSUITABLE`, `UNKNOWN`, stale or restricted results. See the [producer/consumer relationship map](../component-relationships.md#producer-consumer-and-authority-map). |

Both clients expose the same permitted outcomes through the public API, which
routes to Adithya Gunawardana's private service under the shared role-to-permission
...
keep Adithya Gunawardana's business behavior in its own internal service, preserve
...
Adithya Gunawardana (Member 3) owns:
...
biodiversity presentation (Ushan Srinuka (Member 1)), environmental retrieval and deterministic
activity suitability (Sanuda Abeysinghe (Member 2)), operational
restrictions/approval/execution (Wanshaja Sooriyabandara (Member 4)), or the public authorization
...
| Business workflow | A durable Adithya Gunawardana (Member 3) request/assessment process with shared workflow ID, initiator, business status, result version, timestamps and links to relevant records. This exists independently of an AI run and is defined in the shared [workflow contract](../workflows.md). |
...
| Candidate experience | A destination/activity/offering sourced from Ushan Srinuka (Member 1), combined with current schedule and availability, Sanuda Abeysinghe's condition/suitability result, and Wanshaja Sooriyabandara's applicable operational restrictions. |
| Biodiversity prediction context | Optional genuine ML output obtained through Adithya Gunawardana's private IT3091 adapter from the separate IT3091 workstream. A validated result may carry focal species, requested canonical location/area, model and version, prediction time, probability or habitat-suitability interpretation, uncertainty and limitations. It is supplementary context, not evidence of observed presence or safety. |
...
2. `services/api` authenticates/authorizes the caller and routes the request
   to the private Adithya Gunawardana (Member 3) service. That service validates identifiers and
...
2. Relevant marine evidence and deterministic activity suitability are
   obtained from Sanuda Abeysinghe (Member 2). A missing required condition is represented as
   uncertainty; the planner does not fabricate a value.
3. Applicable restrictions are read from Wanshaja Sooriyabandara's authoritative operational
   state.
4. When the objective has a clear biodiversity-context need, the backend may
   request an optional prediction through Adithya Gunawardana's IT3091 adapter. Use only
...
3. Wanshaja Sooriyabandara's operational state must be respected. The planner cannot create
   its own contradictory restriction state.
4. Sanuda Abeysinghe's deterministic suitability result constrains final output. An
...
- retrieve optional biodiversity prediction context through Adithya Gunawardana's
  validated public capability for Ushan Srinuka's destination/activity experience
  surfaces or applicable planner context. Adithya Gunawardana (Member 3) owns its exact route and
...
| Biodiversity context | Where the workflow requests this optional context, show a genuine validated result and its provenance/uncertainty, or the explicit unavailable/invalid state. It never acts as a safety signal or replaces Ushan Srinuka's experience detail presentation contract. |
...
|---|---|
| Ushan Srinuka (Member 1) — Experience & Biodiversity | Candidate destinations, activities, offerings, schedule, availability, experience constraints and optional prediction context. |
| Sanuda Abeysinghe (Member 2) — Marine Conditions & Safety | Sourced conditions, freshness/gaps and deterministic activity-specific suitability. |
| Wanshaja Sooriyabandara (Member 4) — Coastal Operations | Current restrictions/status and, for the assessed operations path, the Safety & Operations proposal. |
| IT3091 biodiversity inference workstream | Adithya Gunawardana (Member 3) sends a validated, privacy-minimal private request and receives a genuine prediction or dependency/schema failure. Adithya Gunawardana (Member 3) exposes only its validated public result capability; Ushan Srinuka (Member 1) consumes that result for the experience-facing screen. |
...
- Ushan Srinuka (Member 1) can consume the validated Adithya Gunawardana (Member 3) public result contract, while
...
- Component work areas on one member branch: [Adithya Gunawardana (Member 3) phase plan](../phases/member-3-phase-plan.md); producer/consumer relationships: [component relationship map](../component-relationships.md); PR and G07 process: [member branch workflow](../member-branch-workflow.md).
