...
2. `services/api` mediates all client-requested map-provider access by
   routing/forwarding public `/api/...` operations to Ushan Srinuka's service.
   React and Flutter call only the public API; provider credentials and
   outbound requests remain in Ushan Srinuka's service. A provider that cannot
...
- Ushan Srinuka (Member 1) can develop its component on the existing single
  ...
  workflow.
- Ushan Srinuka's service owns map requests, provider credentials, response
...
Before the map integration is accepted, the Ushan Srinuka (Member 1) PR and its follow-up
