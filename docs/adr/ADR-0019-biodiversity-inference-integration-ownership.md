...
used by planning as optional contextual data, while Ushan Srinuka (Member 1) remains the
natural owner of how such context is explained in destination/activity
screens. Wanshaja Sooriyabandara (Member 4) owns operational restrictions, assessments and approval;
...
1. **Adithya Gunawardana (Member 3) owns BLUEVERSE's IT3091 integration.** Its
   ...
   capability inside Adithya Gunawardana's own internal service. `services/api` receives
   only the public-route and service-integration wiring. Adithya Gunawardana (Member 3) does not
   train, host or claim ownership of the separate IT3091 model or inference
   service.
2. **Ushan Srinuka (Member 1) owns the experience-facing use.** Its
   `features/experience-biodiversity` branch consumes Adithya Gunawardana's
   agreed public contract to request or display relevant biodiversity
   context in destination/activity experiences. Ushan Srinuka (Member 1) does not implement
   a second inference adapter and does not address the private IT3091 host.
3. **Wanshaja Sooriyabandara (Member 4) is not the adapter owner.** Wanshaja Sooriyabandara (Member 4) may consider validated
...
4. **Keep the boundaries server-side.** React, Flutter and Agentic AI model
   ...
   host. The Adithya Gunawardana (Member 3) component service validates the request, calls the private
...
6. **Implement this integration before G07 as member-feature work.** It is
   ordinary Adithya Gunawardana (Member 3) service integration behind the public API and is required
   for v1. It is not
   an AI agent, RAG pipeline, LLM call, prompt, tool runtime or AI-owned
   workflow. After all four member components pass G07, the Ushan Srinuka (Member 1)
   biodiversity specialist may use a separately allowlisted tool whose
   backend handler calls the approved Adithya Gunawardana (Member 3) contract. The agent still does
...
- Adithya Gunawardana's planner service assembles optional contextual information
  ...
  integration layer.
- Ushan Srinuka (Member 1) still owns the user need: discovery and clear presentation of
  sourced biodiversity information alongside coastal destinations and
  activities. It can implement against the agreed Adithya Gunawardana (Member 3) contract and
...
- Wanshaja Sooriyabandara's scope involves managed operational state and human approval.
  Assigning the biodiversity adapter to Wanshaja Sooriyabandara (Member 4) risks coupling optional
...
- Update the Ushan Srinuka (Member 1) and Adithya Gunawardana (Member 3) component contracts, their work-area
...
- Adithya Gunawardana (Member 3) records evidence for genuine model-backed calls and deterministic
  validation/failure cases. Ushan Srinuka (Member 1) records evidence for rendering sourced
...
- Shared route/DTO decisions are agreed at G00 and cross-checked after the
  feature PRs merge. Ushan Srinuka (Member 1) can use contract fixtures while Adithya Gunawardana's
  adapter is under implementation; fixture success alone is not integration
  acceptance.
- The Sanuda Abeysinghe (Member 2) Open-Meteo adapter and Ushan Srinuka (Member 1) map adapter remain owned by
  their original components. These are separate provider contracts.
- The user's clarification also removes Sanuda Abeysinghe's separately assigned
  device feature. A requested condition period remains an ordinary Sanuda Abeysinghe (Member 2)
  query field; it is distinct from Adithya Gunawardana's itinerary date/time controls.
  Current device assignments are Ushan Srinuka (Member 1) GPS/location discovery, Adithya Gunawardana (Member 3)
  itinerary date/time and Wanshaja Sooriyabandara (Member 4) optional assessment image evidence.
...
### Keep the adapter with Ushan Srinuka (Member 1)
...
planning context with the screen that presents it, while Adithya Gunawardana (Member 3) already
owns the planning service integration. The contracts remain explicit so Ushan Srinuka (Member 1)
can consume Adithya Gunawardana's API without losing ownership of UX.

### Assign the adapter to Wanshaja Sooriyabandara (Member 4)
...
operational/safety decision signal, and Wanshaja Sooriyabandara (Member 4) must not obtain implicit
...
Adithya Gunawardana's feature PR must show a genuine call to the separately supplied
...
contract suitable for Ushan Srinuka (Member 1). Ushan Srinuka's feature PR must show available and
