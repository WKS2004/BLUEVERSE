...
| **Biodiversity ML inference** | The separate IT3091 model integration that produces biodiversity predictions. | It is a domain model integration owned outside the Agentic AI runtime. An LLM must not invent or replace its predictions. |
...
| Biodiversity prediction | Separate IT3091 inference integration mediated by Member 1 backend | Optional genuine prediction with model/version/location/time/provenance/uncertainty. It is not an LLM-generated substitute. |
...
| **Coastal Experience & Biodiversity (Member 1)** | Receives relevant canonical IDs, time/location and information need; returns published offering, schedule, availability, operational context and optional biodiversity information. | Typed sourced report; biodiversity result only when the separate model returns a valid genuine prediction. Read-only tools. | Cannot fabricate availability/prediction, expose unpublished/private data, or treat probability/suitability as observed presence or safety clearance. |
