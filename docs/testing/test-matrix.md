...
| Flutter | `flutter analyze` and machine-mode `flutter test` validate gateway/configuration, build-define and starter widget cases plus `MOB-AUTH-001`–`MOB-AUTH-013` for native Auth requests, credential persistence, refresh recovery, session listing, malformed and incomplete responses, logout and network failures through an in-memory credential store and mock public API; future workflow tests cover the same participating roles with mobile-appropriate interaction; no `apps/mobile/integration_test` directory is checked in yet |
